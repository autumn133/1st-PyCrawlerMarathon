...
class PttcrawlerSpider(scrapy.Spider): #scrapy.Spider必須要是唯⼀值
    name = 'PTTCrawler' #PTTCrawler必須要是唯⼀值
    allowed_domains = ['www.ptt.cc'] #:authority: www.ptt.cc
    start_urls = ['https://www.ptt.cc/bbs/Gossiping/M.1557928779.A.0C1.html']
    cookies = {'over18': '1'} #set-cookie: over18=1; Path=/

    def start_requests(self):  #1.準備傳送請求,#這邊會將網址包裝成 scrapy.Request 而不是 requests
        for url in self.start_urls:
            yield scrapy.Request(url=url, callback=self.parse, cookies=self.cookies)

    def parse(self, response): #4. 傳送回應 ,5. 解析回應的邏輯
