...
class PttcrawlerSpider(scrapy.Spider):
    name = 'PTTCrawler'
    def __init__(self, board='Gossiping'): #整理出要開始爬的最初目標網址
    ...
    def start_requests(self): #對最初的目標網址送出請求
        yield scrapy.Request(url=self.start_urls, callback=self.parse, cookies=self.cookies)

    def parse(self, response): #resposne 取得文章列表,整理後對文章網址逐一以框架內的方式送出請求
        # 取得列表中的清單主體
    ...
    def parse_article(self, response): # resposne 取得文章內容,整理取得目標資料, 送入Item Pipeline
